...
class ApplyCondEff:
    ...
    def modify_predicate_apply_cond_type(self, old_p):
        p = self.modify_predicate(old_p, self.cons_rml)
        if self.cons_cond_type == 'del':
            p.negated = not p.negated
        print(p, "\n")
        return p

    def modify_predicate(self, old_p, mod_p):
                ...
                if new_pred.bdi:
                    if mod_p.nest:
                        ...
                        if not new_pred.bdi[0] and not mod_p.bdi[0]:
                            mod_p.bdi[1][3] = old_p.bdi[1][3]
                            new_pred.bdi[1] = mod_p.bdi[1] + new_pred.bdi[1]
    ...
    def handle_list_comp(self, list_comp_terms, next_cond):
        ...
        for i in range(len(new_preds)):
            new_preds[i] = self.modify_predicate(new_preds[i], list_comp_terms[0])
        return new_preds

    def get_cond_preds(self, cons_cond, next_cond):
                ...
                if type(term) is list:
                    if type(term[0]) is list:
                        if term[0][0] == "COMPOUND":
                            ...
                            cond_preds.extend(self.handle_list_comp(term[0][2:-1], next_cond))
                            continue
    ...
    def create_cond(self, cons_cond, next_cond):
        if cons_cond:
            return self.get_cond_preds(cons_cond, next_cond)
        return None

    def create_conds(self, next_cond):
        new_pos_cond = self.create_cond(self.cons_pos_cond, next_cond)
        new_neg_cond = self.create_cond(self.cons_neg_cond, next_cond)
    ...
    def create_consequent(self, next_cond):
        if type(next_cond) is When:
            cond = self.create_conds(next_cond)
            ...
            f = When(cond, self.modify_predicate_apply_cond_type(next_cond.effect)) 
            return f   
    ...
    while condleft:
        ...
        if next_cond not in processed_conds:
            ...
            for anc_eff in anc_effs._anceffs:
                anc_eff_data = ApplyCondEff(anc_eff, o)
                if anc_eff_data.name in ["mutual-awareness-pos", "mutual-awareness-neg"]:
                    continue
                if anc_eff_data.check_ant_format(next_cond):
                    condleft.append(anc_eff_data.create_consequent(deepcopy(next_cond)))
